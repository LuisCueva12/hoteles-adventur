...
const OFERTAS = [
  {
    ...
    descuento: '33% OFF',
    imagen: 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=1200&q=60&auto=format&fit=crop',
  ...
  },
  {
    ...
    descuento: '25% OFF',
    imagen: 'https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?w=1200&q=60&auto=format&fit=crop',
    validoHasta: '2026-12-31',
    codigo: 'ROMANCE25',
    destacado: false,
  ...
  },
  {
    ...
    descuento: '20% OFF',
    imagen: 'https://images.unsplash.com/photo-1571896349842-33c89424de2d?w=1200&q=60&auto=format&fit=crop',
    validoHasta: '2026-12-31',
    codigo: 'EARLY20',
    destacado: false,
...
function getOfferStatus(validUntil: string) {
  const difference = new Date(`${validUntil}T23:59:59`).getTime() - Date.now()
    ...
    <section className="relative overflow-hidden bg-[linear-gradient(180deg,#fffdf6_0%,#fff8df_48%,#ffffff_100%)] py-20 sm:py-24">
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
          <div>
            <span className="inline-flex items-center gap-2 rounded-full border border-yellow-200 bg-white/80 px-4 py-2 text-[11px] font-bold uppercase tracking-[0.24em] text-yellow-700 shadow-sm backdrop-blur">
            ...
            <h2 className="mt-5 max-w-3xl text-4xl font-bold leading-tight text-slate-900 sm:text-5xl">
              ...
              <span className="block text-yellow-600">y si provocan reservar.</span>
            </h2>
          ...
          <div className="rounded-[28px] border border-slate-200/70 bg-white/85 p-6 shadow-[0_18px_60px_rgba(15,23,42,0.08)] backdrop-blur">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">Ventaja Adventur</p>
          ...
          {OFERTAS.map((oferta) => (
            <article
              ...
              className={`group flex h-full flex-col overflow-hidden rounded-[30px] border bg-white/95 shadow-[0_18px_40px_rgba(15,23,42,0.08)] backdrop-blur transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_26px_50px_rgba(15,23,42,0.14)] ${
                oferta.destacado ? 'border-yellow-300 ring-4 ring-yellow-300/35' : 'border-slate-200/80'
              }`}
            >
              <div className="relative h-64 overflow-hidden">
                ...
                <div className="absolute left-5 top-5 flex max-w-[70%] flex-wrap gap-2">
                  <span className="inline-flex items-center gap-1 rounded-full bg-white/95 px-3 py-1.5 text-[11px] font-bold uppercase tracking-[0.18em] text-slate-900 shadow-sm">
                    {oferta.destacado ? <Star className="h-3.5 w-3.5 fill-yellow-400 text-yellow-500" /> : null}
                    {oferta.etiqueta}
                  </span>
                </div>
                ...
                <div className="absolute inset-x-5 bottom-5">
                  ...
                  <h3 className="mt-2 text-2xl font-bold leading-tight text-white">{oferta.titulo}</h3>
                </div>
              </div>

              <div className="flex flex-1 flex-col p-6">
                <p className="text-base leading-7 text-slate-700">{oferta.descripcion}</p>
                ...
                <div className="mt-6 grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                    ...
                    <p className="mt-2 text-sm font-semibold text-slate-900">{getOfferStatus(oferta.validoHasta)}</p>
                  </div>
                ...
                <div className="mt-6 flex items-center justify-between text-sm text-slate-600">
                  ...
                  <span className="font-semibold text-yellow-700">Condiciones vigentes</span>
                </div>

                <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                  <Link
                    href={`/hoteles?oferta=${oferta.codigo}`}
                    className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-2xl bg-yellow-400 px-6 py-3.5 text-center text-sm font-bold text-slate-950 transition-all duration-300 hover:bg-yellow-500"
                  >
                    Reservar ahora
                  ...
                  <Link
                    href="/hoteles"
                    className="inline-flex min-h-12 items-center justify-center rounded-2xl border border-slate-200 px-5 py-3.5 text-sm font-semibold text-slate-700 transition-all duration-300 hover:border-yellow-300 hover:bg-yellow-50 hover:text-slate-900"
                  >
                    Ver opciones
                  </Link>
                </div>
        ...
        <div className="mt-12 rounded-[28px] border border-slate-200/80 bg-white/90 px-6 py-5 shadow-[0_14px_30px_rgba(15,23,42,0.06)] backdrop-blur sm:px-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <Link
              href="/hoteles"
              className="inline-flex min-h-12 items-center justify-center rounded-2xl border-2 border-yellow-400 px-8 py-3.5 text-sm font-bold text-yellow-700 transition-all duration-300 hover:bg-yellow-400 hover:text-slate-950"
            >
